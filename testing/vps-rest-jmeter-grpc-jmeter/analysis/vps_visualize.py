...
Reads raw JMeter sample results (rest-grpc-report_vps.csv, containing
one row per request across all 20 Thread Groups: 15 GET + 5 POST),
both REST API and gRPC tested via JMeter on a VPS pair (server +
client), and produces:
...
Expects rest-grpc-report_vps.csv to sit in ../results/ relative to
this script.
"""
...
RESULTS_CSV = SCRIPT_DIR.parent / "results" / "rest-grpc-report_vps.csv"
...
# filenames and must be told apart when read in isolation.
SCENARIO_LABEL = "Lingkungan Pengujian: VPS"
SERVER_LABEL = "Server (REST API dan gRPC): e2-standard-8, 8 vCPU, 32 GB RAM"
CLIENT_LABEL = "Client (REST API dan gRPC via JMeter): e2-standard-4, 4 vCPU, 16 GB RAM"
...
def load_raw_samples() -> pd.DataFrame:
    ...
    if not RESULTS_CSV.exists():
        raise FileNotFoundError(
            f"Tidak ketemu {RESULTS_CSV}. Pastikan rest-grpc-report_vps.csv "
...
def plot_response_time_overlay(df: pd.DataFrame, size: str):
    """GET overlay chart: one line per combination, x-axis = time since test start (bucketed into 1-second windows), y-axis = mean response time."""
...
def compute_post_summary(df: pd.DataFrame) -> pd.DataFrame:
    """Compute mean/median/p90/p95/p99/min/max/throughput per POST label."""
    rows = []
    for label, group in df.groupby("label"):
...
def plot_post_response_time_overlay(df: pd.DataFrame):
    """POST overlay chart: one line per combination, with markers on each point."""
    fig, ax = plt.subplots(figsize=(10, 6))
